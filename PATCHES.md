...
| 8 | New resource: fmc_mze_network_groups (originally fmc_network_groups_safe; renamed 2026-06-04) | multiple new files | None — entirely new files |
...
## Patch 8 — New resource: fmc_mze_network_groups

> **Renamed 2026-06-04** (during Patch 12 work): `fmc_network_groups_safe` →
> `fmc_mze_network_groups`. The `mze_` prefix marks the resource as a
> Mechanize-fork addition (matches the convention introduced by Patch 12).
> The Go type also became `MzeNetworkGroupsResource`. This is a breaking
> change for any user of the old name — there is no Terraform-level
> aliasing.

### What it does

`fmc_mze_network_groups` is a drop-in replacement for `fmc_network_groups`
...
- `internal/provider/resource_fmc_mze_network_groups.go` — resource implementation
- `examples/resources/fmc_mze_network_groups/resource.tf` — required by tfplugindocs
- `docs/resources/mze_network_groups.md` — generated by `go generate`
...
- `tests/network_groups_safe/` — verifies `fmc_mze_network_groups` at 1000 groups + rules (directory name kept for historical continuity)
...
from upstream resources at the HCL level. Patch 8 was renamed to follow
this convention on the same day Patch 12 landed (see the Patch 8 entry
above for the breaking-change note).
...
6. **Patch 8** — `resource_fmc_mze_network_groups.go` is not generated;
